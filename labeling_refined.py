...
def prenp(args, paths, noise_labels):
    ...
    refiner = CleanSampleRefiner(
        ...
        device=device,
        confidence_threshold=0.7,  # Start with lower threshold
    ...
    print("\n" + "="*80)
    print("ITERATIVE CLEAN-SAMPLE REFINEMENT")
    print("="*80)
    ...
    for iteration in range(n_iterations):
        ...
        scheduler = get_scheduler(args.ds, optimizer, args.ep // n_iterations)
        
        # Train for fewer epochs per iteration
        epochs_per_iteration = max(10, args.ep // n_iterations)
        ...
        print(f"\nIteration {iteration + 1} completed. Best Val Acc: {val_accuracy_best:.4f}, "
              f"Best Test Acc: {best_test_acc:.4f}")
        
        # Update clean mask for next iteration (except last iteration)
        if iteration < n_iterations - 1:
            print(f"\nRefining clean sample set...")
            
            # Extract features using best model from this iteration
            features, labels_array, _ = refiner.extract_features(best_model, train_eval_loader)
            
            # Clustering-based filtering
            cluster_mask, cluster_conf = refiner.cluster_based_filtering(
                features, noise_labels[train_indices], iteration
            )
            
            # Model-based filtering
            model_mask, model_probs = refiner.model_based_filtering(
                best_model, train_eval_loader, noise_labels[train_indices]
            )
            
            # Combine filters
            clean_mask, sample_weights = refiner.combine_filters(
                cluster_mask, model_mask, cluster_conf, model_probs
            )
            
            # Early stopping if clean set is too small or too large
            clean_ratio = clean_mask.mean()
            if clean_ratio < 0.3:
                print(f"  Warning: Clean set too small ({clean_ratio:.1%}). "
                      f"Relaxing filters...")
                refiner.confidence_threshold = max(0.5, refiner.confidence_threshold - 0.1)
            elif clean_ratio > 0.95:
                print(f"  Warning: Clean set too large ({clean_ratio:.1%}). "
                      f"Tightening filters...")
                refiner.confidence_threshold = min(0.9, refiner.confidence_threshold + 0.1)
