...
def five_cut_transform(dataname, crop_ratio=0.8, use_attention=False, device='cuda'):
    """
    Create transform for multi-crop augmentation
    
    Args:
        dataname: dataset name
        crop_ratio: ratio of crop size to image size
        use_attention: if True, use attention-based cropping instead of FiveCrop
        device: device for attention model (only used if use_attention=True)
    """
    ...
    if use_attention:
        from utils.attention_crop import attention_based_crop_transform
        return attention_based_crop_transform(dataname, crop_ratio=crop_ratio, num_crops=5, device=device)
    
    # Original FiveCrop implementation
    mean_, std_, crop_size_ = mean[dataname], std[dataname], crop_size[dataname]
    if dataname == 'clothing1m':
        return transforms.Compose([
