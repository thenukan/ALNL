...
def load_human_noisy_labels(noise_file_path, noise_type):
    """
    Load human-annotated noisy labels from .pt file
    
    Args:
        noise_file_path: Path to .pt file with noisy labels
        noise_type: Type of noise to load (e.g., 'worse_label', 'aggre_label', 'random_label1', etc.)
    
    Returns:
        noisy_labels: Numpy array of noisy labels
    """
    import torch
    
    if not os.path.exists(noise_file_path):
        raise FileNotFoundError(f"Human noise label file not found: {noise_file_path}")
    
    print(f"Loading human-annotated noisy labels from: {noise_file_path}")
    noise_data = torch.load(noise_file_path, weights_only=False)
    
    if not isinstance(noise_data, dict):
        raise ValueError("Expected .pt file to contain a dictionary")
    
    available_types = list(noise_data.keys())
    print(f"  Available noise types: {available_types}")
    
    if noise_type not in noise_data:
        raise ValueError(f"Noise type '{noise_type}' not found in .pt file. Available: {available_types}")
    
    # Load the specified noise type
    noisy_label_data = noise_data[noise_type]
    if isinstance(noisy_label_data, torch.Tensor):
        noisy_labels = noisy_label_data.numpy()
    else:
        noisy_labels = np.array(noisy_label_data)
    
    # Calculate noise rate if clean labels available
    if 'clean_label' in noise_data:
        clean_label_data = noise_data['clean_label']
        if isinstance(clean_label_data, torch.Tensor):
            clean_labels = clean_label_data.numpy()
        else:
            clean_labels = np.array(clean_label_data)
        
        noise_rate = 1 - np.mean(clean_labels == noisy_labels)
        print(f"  Using noise type: {noise_type}")
        print(f"  Overall noise rate: {noise_rate:.4f} ({noise_rate*100:.2f}%)")
    else:
        print(f"  Using noise type: {noise_type}")
    
    return noisy_labels.reshape(-1)
...
def main(args, paths):
    ...
    # Generate or load noisy labels based on args.use_human_labels
    if args.use_human_labels:
        print("="*80)
        print("Using Human-Annotated Noisy Labels")
        print("="*80)
        noise_labels = load_human_noisy_labels(args.human_label_path, args.human_label_type)
        print("="*80)
    else:
        print("="*80)
        print("Generating Synthetic Noisy Labels")
        print("="*80)
...
if __name__ == '__main__':
    ...
    # Human-annotated noisy label arguments
    parser.add_argument('--use_human_labels', help='use human-annotated noisy labels instead of synthetic', 
                        action='store_true')
    parser.add_argument('--human_label_path', help='path to .pt file with human-annotated noisy labels',
                        default='./data/CIFAR-10_human.pt', type=str, required=False)
    parser.add_argument('--human_label_type', help='type of human noise to use (worse_label, aggre_label, random_label1, etc.)',
                        default='worse_label', type=str, 
